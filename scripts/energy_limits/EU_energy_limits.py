...
def parse_args():
    parser = argparse.ArgumentParser(description='Run EU network-energy-cap experiment on multiple hosts.')
...
if __name__ == "__main__":
    ...
    if args.cancel:
        ...
        if max_extra_network_energy_fracs is not None:
            load_shifts_dir = f'/nfs/obelix/raid/jrmurillo/GreenVideoModel_data/load_shifts/EU_{year}_energy_limit/' if args.save_load_shifts else None
        else:
            load_shifts_dir = f'/nfs/obelix/raid/jrmurillo/GreenVideoModel_data/load_shifts/EU_{year}/' if args.save_load_shifts else None

        print("Starting EU network-energy-cap experiment on specified hosts and months.")
        ...
        for host, month in list(zip(hosts, months)):
            ...
            results_dir = f'results/EU_energy_limit_year_{year}/'
            output_dir = f'output/experiments/EU_energy_limit_year_{year}/'

            os.makedirs(output_dir, exist_ok=True)
            os.makedirs(results_dir, exist_ok=True)
            results_file_flag = f'--results_file {results_dir}EU_energy_limit_{host}_month_{month:02d}.csv'
            ...
            COMMAND = f"source ~/python-greenstreaming/bin/activate && cd ~/GreenVideoModel && nohup python3 scripts/energy_limits/energy_limit_worker.py --region_counts_file data/region_counts/region_counts_eu.json  --dirs {dir_}  {load_shifts_flag}  {energy_limit_flag} {server_power_factor_flag} {results_file_flag} > {output_dir}EU_energy_limit_{host}_{month:02d}.out 2>&1 &"
