...
def parse_args():
    parser = argparse.ArgumentParser(description='Run EU carbon experiment on multiple hosts.')
    ...
    # parser.add_argument('--load_shifts_dir', type=str, default=None, help='Directory to save the load shifts data (default: None)')
...
if __name__ == "__main__":
    ...
    if args.cancel:
        ...
        if peak_bit_load_file:
            load_shifts_dir = f'/nfs/obelix/raid/jrmurillo/GreenVideoModel_data/load_shifts/EU_{year}_peak/' if args.save_load_shifts else None
        else:
            load_shifts_dir = f'/nfs/obelix/raid/jrmurillo/GreenVideoModel_data/load_shifts/EU_{year}/' if args.save_load_shifts else None

        # print(f"Load shifts directory: {load_shifts_dir}")
        print("Starting EU carbon experiment on specified hosts and months.")
        # sys.exit(0)

        for host, month in list(zip(hosts, months)):
        # for host, month in list(zip(hosts, months))[0:1]:
        
            dir_ = f'/nfs/obelix/raid2/jrmurillo/ECORInfo_EU_{year}/{month:02d}'
            ...
            if peak_bit_load_file:
                ...
                results_dir = f'results/EU_carbon_year_{year}_peak/'
                output_dir = f'output/experiments/EU_carbon_year_{year}_peak/'
                ...
                peak_bit_load_flag = ''
                results_dir = f'results/EU_carbon_year_{year}/'
                output_dir = f'output/experiments/EU_carbon_year_{year}/'

            os.makedirs(output_dir, exist_ok=True)
            os.makedirs(results_dir, exist_ok=True)
            results_file_flag = f'--results_file {results_dir}EU_carbon_{host}_month_{month:02d}.csv'
            ...
            if args.server_power_factor is not None:
                ...
                # server_power_factor = 1.0

            COMMAND = f"source ~/python-greenstreaming/bin/activate && cd ~/GreenVideoModel && nohup python3 scripts/experiment_scripts/carbon_worker.py --region_counts_file data/region_counts/region_counts_eu.json  --dirs {dir_}  {load_shifts_flag}  {peak_bit_load_flag} {server_power_factor_flag} {results_file_flag} > {output_dir}EU_carbon_{host}_{month:02d}.out 2>&1 &"
            ...
            #     COMMAND = f"source ~/python-greenstreaming/bin/activate && cd ~/GreenVideoModel && nohup python3 scripts/experiment_scripts/carbon_worker.py --region_counts_file data/region_counts/region_counts_eu.json  --dirs {dir_}  --load_shifts_dir {load_shifts_dir_month}  --results_file {results_dir}EU_carbon_{host}_month_{month:02d}.csv > {output_dir}EU_carbon_{host}_{month:02d}.out 2>&1 &"
            # else:
            #     COMMAND = f"source ~/python-greenstreaming/bin/activate && cd ~/GreenVideoModel && nohup python3 scripts/experiment_scripts/carbon_worker.py --region_counts_file data/region_counts/region_counts_eu.json  --dirs {dir_} --results_file {results_dir}EU_carbon_{host}_month_{month:02d}.csv > {output_dir}EU_carbon_{host}_{month:02d}.out 2>&1 &"
            ######
