...
def parse_args():
    parser = argparse.ArgumentParser(description='Run EU carbon server-power sensibility analysis on multiple hosts.')
...
if __name__ == "__main__":
    ...
    if args.cancel:
        ...
        print("Starting EU carbon server-power sensibility analysis on specified hosts and months.")
        ...
        for host, month in list(zip(hosts, months)):
            ...
            if peak_bit_load_file:
                ...
                results_dir = f'results/sensibility_analysis/EU/EU_carbon_sensibility_year_{year}_peak/'
                output_dir = f'output/experiments/sensibility_analysis/EU/EU_carbon_sensibility_year_{year}_peak/'
            else:
                peak_bit_load_flag = ''
                results_dir = f'results/sensibility_analysis/EU/EU_carbon_sensibility_year_{year}/'
                output_dir = f'output/experiments/sensibility_analysis/EU/EU_carbon_sensibility_year_{year}/'

            os.makedirs(output_dir, exist_ok=True)
            os.makedirs(results_dir, exist_ok=True)
            # results_dir_flag = f'--results_file {results_dir}EU_carbon_{host}_month_{month:02d}.csv'

            results_dir_flag = f'--results_dir {results_dir}'

            COMMAND = f"source ~/python-greenstreaming/bin/activate && cd ~/GreenVideoModel && nohup python3 scripts/experiment_scripts/carbon_worker_sensibility.py --region_counts_file data/region_counts/region_counts_eu.json  --dirs {dir_}  {load_shifts_flag}  {peak_bit_load_flag} {server_power_factor_flag} {results_dir_flag} > {output_dir}EU_carbon_{host}_{month:02d}.out 2>&1 &"
